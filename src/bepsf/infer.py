...
def optimize(gridpsf,
    ...
    p_init = {
        ...
        "lnlenx": np.float64(
            0.5),  # np.float64() required so that pytree leaf has "shape"
    ...
    def objective(p):
        fluxes = jnp.exp(jnp.r_[p['lnfluxes'][:idx_anchor],
                                lnfluxes_guess[idx_anchor],
                                p['lnfluxes'][idx_anchor + 1:]])
        xcenters = jnp.r_[p['xcenters'][:idx_anchor],
                          xcenters_guess[idx_anchor],
                          p['xcenters'][idx_anchor + 1:]]
        ycenters = jnp.r_[p['ycenters'][:idx_anchor],
                          ycenters_guess[idx_anchor],
                          p['ycenters'][idx_anchor + 1:]]
        lenx, leny, amp2, mu = jnp.exp(p['lnlenx']), jnp.exp(
            p['lnleny']), jnp.exp(2 * p['lnamp']), jnp.exp(p['lnmu'])
        return -gridpsf.log_likelihood(
            fluxes, xcenters, ycenters, lenx, leny, amp2, mu,
            image_obs.X1d[~mask1d], image_obs.Y1d[~mask1d],
            image_obs.Z1d[~mask1d], image_obs.Zerr1d[~mask1d])
...
def numpyro_model(gridpsf, image_obs, popt, fix_mean=True):
    ...
    lnlenx = numpyro.sample(
        "lnlenx",
        dist.Uniform(low=0 * (popt['lnlenx'] - 2), high=popt['lnlenx'] + 2))
    lnleny = numpyro.sample(
        "lnleny",
        dist.Uniform(low=0 * (popt['lnleny'] - 2), high=popt['lnleny'] + 2))
    lna = numpyro.sample(
        "lna", dist.Uniform(low=popt['lnamp'] - 5, high=popt['lnamp'] + 2))
    lenx, leny, amp2 = jnp.exp(lnlenx), jnp.exp(lnleny), jnp.exp(2 * lna)
    if fix_mean:
        mu = 0.
    else:
        lnmu = numpyro.sample(
            "lnmu", dist.Uniform(low=popt['lnmu'] - 5, high=popt['lnmu'] + 2))
        mu = jnp.exp(lnmu)

    # omit anchored source (otherwise the sampling is slow)
    f = numpyro.sample(
        "f",
        dist.Uniform(low=0 * popt['lnfluxes_drop'],
                     high=2 * jnp.exp(popt['lnfluxes_drop'])))
    x = numpyro.sample(
        "x",
        dist.Uniform(low=popt['xcenters_drop'] - 1.,
                     high=popt['xcenters_drop'] + 1))
    y = numpyro.sample(
        "y",
        dist.Uniform(low=popt['ycenters_drop'] - 1.,
                     high=popt['ycenters_drop'] + 1))
    idx_anchor = image_obs.idx_anchor
    fluxes = jnp.r_[f[:idx_anchor],
                    jnp.exp(popt['lnfluxes'][idx_anchor]), f[idx_anchor:]]
    xcenters = jnp.r_[x[:idx_anchor], popt['xcenters'][idx_anchor],
                      x[idx_anchor:]]
    ycenters = jnp.r_[y[:idx_anchor], popt['ycenters'][idx_anchor],
                      y[idx_anchor:]]

    # masked pixels are ignored in likelihood evaluation
    mask1d = image_obs.mask1d
    gploglike = gridpsf.log_likelihood(fluxes, xcenters, ycenters, lenx, leny,
                                       amp2, mu, image_obs.X1d[~mask1d],
                                       image_obs.Y1d[~mask1d],
                                       image_obs.Z1d[~mask1d],
